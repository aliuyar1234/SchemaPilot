...
- 2026-02-18 — Started `TASKLIST_NEXT_V2`: completed `V2-0005` (strict config schema v2 with config-file loading + unknown-key fail + redaction contract) and `V2-0003` (`schemapilot doctor` deterministic preflight checks with structured pass/fail report output).
...
- 2026-02-18 — Fixed CI/release manifest false negatives by excluding build-generated `*.egg-info` files from manifest scope (these are mutated by `pip install -e` in GitHub Actions), then regenerated `MANIFEST.sha256`.
